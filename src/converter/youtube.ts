...
const getTrack = async (id: string) => {
  ...
  const script = parse(response.data)
    ...
    .find((script) =>
      script.childNodes.find((node) => node.rawText.includes("responseText"))
    );
  ...
  if (script) {
    ...
    if (info) {
      ...
      try {
        const data =
          json.contents.twoColumnWatchNextResults.results.results.contents[1]
        ...
        if (data.rows && data.rows.length > 0) {
          const song = data.rows.find(
            (row: any) =>
              ...
              row.metadataRowRenderer.title.simpleText === "Song",
          );
          const artist = data.rows.find(
            (row: any) =>
              row.metadataRowRenderer &&
              row.metadataRowRenderer.title.simpleText === "Artist",
          );
        ...
        const data = TRACK.data.filter((track) =>
          title.includes(track.ART_NAME.toLowerCase())
        );
        if (data[0]) {
          return TRACK.data[0];
        }
