...
export const getUrlParts = async (
  url: string,
  setToken = false,
): Promise<urlPartsType> => {
  ...
  switch (site[0]) {
    ...
    case "spotify":
      ...
      return {
        type: ("spotify-" + spotifyUrlParts.type) as any,
        id: (spotifyUrlParts as any).id,
      };

    case "tidal":
      const tidalUrlParts = url.split(/\/(\w+)\/(\d+|\w+-\w+-\w+-\w+-\w+)/);
      return {
        type: ("tidal-" + tidalUrlParts[1]) as any,
        id: tidalUrlParts[2],
      };
...
export const parseInfo = async (url: string) => {
  ...
  switch (info.type) {
        ...
        artistAlbums.data.map((album) => {
          return async () => {
            if (album.ARTISTS.find((a) => a.ART_ID === info.id)) {
              ...
              tracks = [
                ...tracks,
                ...albumTracks.data.filter((t) => t.ART_ID === info.id),
              ];
            }
          };
        }),
    ...
    case "spotify-album":
      const [spotifyAlbumInfo, spotifyTracks] = await spotify.album2deezer(
        info.id,
      );
    ...
    case "spotify-playlist":
      const [spotifyPlaylistInfo, spotifyPlaylistTracks] = await spotify
        .playlist2Deezer(info.id);
    ...
    case "tidal-album":
      const [tidalAlbumInfo, tidalAlbumTracks] = await tidal.album2deezer(
        info.id,
      );
    ...
    case "tidal-playlist":
      const [tidalPlaylistInfo, tidalPlaylistTracks] = await tidal
        .playlist2Deezer(info.id);
