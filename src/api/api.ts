import { request, requestGet, requestLight, requestPublicApi } from "./request";
import type {
  albumTracksType,
  albumType,
  albumTypePublicApi,
 ...
 */
export const getTrackInfoPublicApi = (
  sng_id: string,
): Promise<trackTypePublicApi> => requestPublicApi("/track/" + sng_id);

/**
 * @param {String} alb_id album id
 */
export const getAlbumInfoPublicApi = (
  alb_id: string,
): Promise<albumTypePublicApi> => requestPublicApi("/album/" + alb_id);

/**
 * @param {String} sng_id song id
 */
export const getTrackInfo = (sng_id: string): Promise<trackType> =>
  request({ sng_id }, "song.getData");

/**
 * @param {String} sng_id song id
 */
export const getLyrics = (sng_id: string): Promise<lyricsType> =>
  request({ sng_id }, "song.getLyrics");

/**
 * @param {String} alb_id album id
 */
export const getAlbumInfo = (alb_id: string): Promise<albumType> =>
  request({ alb_id }, "album.getData");
...
export const getArtistInfo = (art_id: string): Promise<artistInfoType> =>
  request(
    { art_id, filter_role_id: [0], lang: "en", tab: 0, nb: -1, start: 0 },
    "artist.getData",
  );
 ...
 */
export const searchAlternative = (
  artist: string,
  song: string,
  nb: number = 10,
): Promise<searchType> =>
...
export const getUser = async (): Promise<userType> =>
  requestGet("user_getInfo");

/**
 * Get list of channles
 */
export const getChannelList = async (): Promise<channelSearchType> =>
  request({}, "search_getChannels");
 ...
 */
export const getPlaylistChannel = async (
  page: string,
): Promise<playlistChannelType> => {
  const gateway_input = {
    page,
    version: "2.3",
    support: {
