...
export const writeMetadataMp3 = (
  ...
  if (album) {
    ...
    if (RELEASE_DATES) {
      writer.setFrame("TYER", RELEASE_DATES[0]).setFrame(
        "TDAT",
        RELEASE_DATES[2] + RELEASE_DATES[1],
      );
    }
    writer
  ...
  if (track.DISK_NUMBER) {
    const TRACK_NUMBER = track.TRACK_NUMBER.toLocaleString("en-US", {
  ...
  if (track.SNG_CONTRIBUTORS && !Array.isArray(track.SNG_CONTRIBUTORS)) {
    if (track.SNG_CONTRIBUTORS.main_artist) {
      writer.setFrame(
        "TCOP",
        `${RELEASE_DATES ? RELEASE_DATES[0] + " " : ""}${
          track.SNG_CONTRIBUTORS.main_artist[0]
        }`,
      );
    }
    ...
    if (track.SNG_CONTRIBUTORS.producer && track.SNG_CONTRIBUTORS.engineer) {
      writer.setFrame("TXXX", {
        ...
        value: track.SNG_CONTRIBUTORS.producer.concat(
          track.SNG_CONTRIBUTORS.engineer,
        ).join(", "),
      });
    }
  }
