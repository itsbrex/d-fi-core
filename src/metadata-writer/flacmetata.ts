...
export const writeMetadataFlac = (
  ...
  if (album) {
    const TOTALTRACKS = album.nb_tracks.toLocaleString("en-US", {
    ...
    flac.setTag(
      `COMPILATION=${album.artist.name.match(/various/i) ? "1" : "0"}`,
    );
  }
  ...
  if (track.SNG_CONTRIBUTORS && !Array.isArray(track.SNG_CONTRIBUTORS)) {
    if (track.SNG_CONTRIBUTORS.main_artist) {
      flac.setTag(
        `COPYRIGHT=${RELEASE_YEAR ? RELEASE_YEAR + " " : ""}${
    ...
    if (track.SNG_CONTRIBUTORS.publisher) {
      flac.setTag(
        "ORGANIZATION=" + track.SNG_CONTRIBUTORS.publisher.join(", "),
      );
    }
